...
import com.mycompany.core.fetch.FetchPlanProperty;
...
public class SecureEntitySerializerImpl implements SecureEntitySerializer {
    ...
    public SecureEntityView serialize(Object entity, FetchPlan fetchPlan) {
        ...
        for (FetchPlanProperty property : fetchPlan.getProperties()) {
            ...
            serializeReference(view, wrapper, entityClass, property);
        }

        return view;
    }

    private void serializeReference(SecureEntityView view, BeanWrapper wrapper, Class<?> entityClass, FetchPlanProperty property) {
        String attr = property.getName();
        ...
        if (refValue == null) {
            ...
            view.put(
                attr,
                collection.stream().map(item -> serialize(item, property.getFetchPlan()).getValues()).filter(Objects::nonNull).toList()
            );
        } else if (refValue.getClass().isArray()) {
            view.put(attr, serializeArray(refValue, property.getFetchPlan()));
        } else {
            view.put(attr, serialize(refValue, property.getFetchPlan()).getValues());
        }
    }
